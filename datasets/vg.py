...
class VGRelDetection(torch.utils.data.Dataset):
    def __init__(self, img_set, img_folder, scene_graphs_anno_file, transforms, num_queries, use_all_text_labels = None, adding_OI = False):
        ...
        if self.use_all_text_labels:
            with open("/PATH/TO/jacob/OCN/datasets/vg_keep_names_v2.json", "r") as f:
            ...
            if not adding_OI:
                keep_names_file = "/PATH/TO/jacob/OCN/datasets/vg_keep_names_v1_no_lias_freq.json"
            # else:
...
def build(image_set, args):
    ...
    if args.dataset_file == 'vg':
        if args.use_aliases:
            ...
            # PATHS = {'pretrain': (root / 'images' , root / 'annotations' / 'scene_graphs_preprocess.json')}
...
def vg_preprocess(dataset_path, IoU_thre, num_queries = 100, save_preprocess = False):
    ...
    # if save_preprocess:
    #     with open(dataset_path / anno_file_name.replace('.', '_preprocess_v1test.'), 'w') as outfile:
    #         # '_preprocessv2.' '_preprocess_greater5.'  '_no_alias_test_nofiltering_relations.'
    #         json.dump(VG_scene_graph, outfile)

# Greater than or equal to 2
# rel_keep: 10991
# obj_keep: 35772
# How many relationships are left after filtering? 1927018
# How many objects are left after filtering? 2540676
# Greater than or equal to 5
# rel_keep: 4706
# obj_keep: 15744
# How many relationships are left after filtering? 1878572
# How many objects are left after filtering? 2479945
