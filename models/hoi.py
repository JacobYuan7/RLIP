...
class RLIP_ParSe(nn.Module):
    def __init__(self, backbone, transformer, num_queries, contrastive_align_loss = False, 
                       ...
                       use_no_verb_token = False, pseudo_verb = False):        
        super().__init__()
        self.num_queries = num_queries
        self.transformer = transformer
        hidden_dim = transformer.d_model
...
class RLIP_ParSeD(nn.Module):
    def __init__(self, backbone, transformer, num_queries, num_feature_levels, 
                       ...
                       subject_class=False, verb_curing=False, masked_entity_modeling=None, pseudo_verb=False, matcher = None):
        super().__init__()
...
class ParSeD(nn.Module):
    ...
    def forward(self, samples: NestedTensor,  **kwargs):
        ...
        if self.verb_curing:
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1], 'curing_score':curing_score}
        else:
            out = {'pred_obj_logits': outputs_obj_class[-1], 'pred_verb_logits': outputs_verb_class[-1],
                'pred_sub_boxes': outputs_sub_coord[-1], 'pred_obj_boxes': outputs_obj_coord[-1]}
        if self.aux_loss:
            if self.verb_curing:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord, curing_score)
            else:
                out['aux_outputs'] = self._set_aux_loss(outputs_obj_class, outputs_verb_class,
                                                        outputs_sub_coord, outputs_obj_coord)
        if self.two_stage:
    ...
    @torch.jit.unused
    def _set_aux_loss(self, outputs_obj_class, outputs_verb_class, outputs_sub_coord, outputs_obj_coord, curing_score = None):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        if curing_score is None:
