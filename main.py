...
def main(args):
    ...
    if args.resume:
        ...
        if args.DDETRHOI or args.ParSeD or args.RLIP_ParSeD:
            checkpoint = utils.filter_ckpt_query_embed(checkpoint, num_queries = args.num_queries)
        elif args.ParSe or args.RLIP_ParSe:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.hoi:
            if hasattr(model_without_ddp.transformer, 'text_encoder'):
                ...
                if epoch<=0 or epoch>=55 or args.frozen_vision:
                    test_stats = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, args.subject_category_id, device, args)
                    coco_evaluator = None
        elif args.coco:
